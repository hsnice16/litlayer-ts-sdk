...
export class AxiosHttpClient implements IHttpClient {
   ...
   private parseResponse<T>(responseData: any, status: number, path?: string): T {
      ...
      if (!apiData.success) {
         throw new LitlayerApiError(
            apiData.code,
            getAPIErrorMsg(apiData.code, apiData.msg),
            apiData as any,
         );
      }
      return apiData.data;
   ...
   private handleError(error: any, path?: string): never {
      if (axios.isAxiosError(error)) {
         const axiosError = error as AxiosError<
            GenericAPIResponse<any> & { code: number; msg: string }
         >;
         ...
         if (response) {
            ...
            if (responseData && typeof responseData === 'object') {
               apiCode =
                  typeof responseData.code === 'number' ? responseData.code : response.status;
               apiMsg =
            ...
            throw new HttpError(
               0,
               `No response received for request to ${path}: ${axiosError.message}`,
            );
         } else {
