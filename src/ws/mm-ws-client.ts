...
export class MmWsClient extends UserWsClient {
  ...
  protected _mmHandlers: Partial<
    Record<MmClientHandlerKeys, MmWsHandlerCallback[]>
  > = {};
  ...
  constructor(
    ...
    const initialMmHandlers: Record<
      MmClientHandlerKeys,
      MmWsHandlerCallback[]
    > = {} as Record<MmClientHandlerKeys, MmWsHandlerCallback[]>;
    ...
    try {
      ...
      const err =
        error instanceof Error
          ? error
          : new Error("MM WebSocket login failed during onOpen");
  ...
  private isMmOperationResponse(m: any): m is MmPureOperationResponse {
    ...
    return (
      ...
      (m.result === MM_WS_LOGIN_RESPONSE_RESULT ||
        m.result === MM_WS_POST_RESPONSE_RESULT)
    );
  }
  ...
  removeMmHandler(
    ...
    if (callbacks) {
      this._mmHandlers[handlerFor] = callbacks.filter(
