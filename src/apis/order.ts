...
export class Order {
  ...
  async queryOpenOrders(
    ...
    return this.internalHttpClient.get<
      QueryResponse<OrderQueryOpenOrdersResponseList>
    >(urlPath, queryParams, extraHeaders);
  }
  ...
  async cancels(
    ...
    const payload: GenericObject<any> = {};
    if (validatedPayload.order_nos)
      payload.order_no = validatedPayload.order_nos; // API expects order_no (singular) for array
  ...
  async create(
    ...
    if (type === OrderType.MARKET) {
      price = getSlippageAdjustedPrice(
        price,
        slippage,
